...
class GenAI:
    ...
    def generate_text(self, prompt, 
                    instructions='You are a helpful AI named Jarvis', 
                    image_paths_list=None, 
                    model="gpt-4o-mini", 
                    output_type='text', 
                    temperature=1):
        """
        Generate text completion using OpenAI's API with optional image input.
        
        This function sends a prompt to the OpenAI API and returns the generated response.
        It supports text-only prompts as well as multimodal prompts with images. The function
        can be configured with system instructions, model selection, and response formatting.
        
        Parameters
        ----------
        prompt : str
            The user input or query for the AI to respond to.
            
        instructions : str, optional
            System instructions that define the AI's behavior and persona.
            Default: 'You are a helpful AI named Jarvis'
            
        image_paths_list : list of str, optional
            List of file paths to images to include with the prompt. Images will be
            base64 encoded and sent as part of the request. Default: None
            
        model : str, optional
            The OpenAI model identifier to use for generation.
            Default: 'gpt-4o-mini'
            
        output_type : str, optional
            Format for the response: 'text' for natural language or 'json_object' for
            structured data. Default: 'text'
            
        temperature : float, optional
            Controls randomness in the response. Higher values (e.g., 1.0) make output
            more random, while lower values (e.g., 0.2) make it more deterministic.
            Range: 0.0 to 2.0. Default: 1
        
        Returns
        -------
        str
            The generated text response with code block markers removed.
        ...
        ...     image_paths_list=["path/to/image.jpg"]
        ... )
        
        >>> # Structured JSON response
        >>> response = generate_text(
        ...     "List the top 3 planets by size",
        ...
        if image_paths_list:
            image_urls = [f"data:image/jpeg;base64,{self.encode_image(image_path)}" 
    ...
    def read_pdf(self, file_path):
        ...
        This function opens a PDF file at the specified path, reads through all pages,
        and extracts the text content into a single string.
        
        Args:
            file_path (str): The path to the PDF file to be read.
            
        Returns:
            str: A string containing all text extracted from the PDF file.
    ...
    def read_docx(self, file_path):
        """
        Read and extract text from a Word document (.docx) file.
        
        This function opens a Word document at the specified path, reads through all paragraphs,
        and extracts the text content into a single string with paragraphs separated by newlines.
        
        Args:
            file_path (str): The path to the Word document to be read.
            
        Returns:
            str: A string containing all text extracted from the document with paragraphs 
